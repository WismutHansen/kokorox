...
struct Cli {
    ...
    /// Input is phonemes instead of text - bypasses text-to-phoneme conversion
    /// When enabled, the input is treated as phonemes and sent directly to the TTS model
    #[arg(
        long = "phonemes",
        help = "Input is phonemes instead of text - bypasses text-to-phoneme conversion",
        default_value_t = false
    )]
    phonemes: bool,
...
fn utf8_safe_custom_segmentation(text: &str, language: &str, verbose: bool) -> Vec<String> {
    ...
    while let Some((_i, ch)) = chars.next() {
        ...
        let is_sentence_ending = if language.starts_with("zh")
        ...
        if is_sentence_ending {
            ...
            if should_split {
                ...
                if !trimmed.is_empty() {
                    ...
                    if verbose {
                        println!(
                            "CUSTOM SEGMENTATION: Added sentence: {}",
                        ...
                        for (i, c) in sentences.last().unwrap().char_indices() {
                            if !c.is_ascii() {
                                println!(
                                    "  Preserved char at {}: '{}' (Unicode: U+{:04X})",
                                    i, c, c as u32
                                );
                            }
                        }
    ...
    if !remaining.is_empty() {
        ...
        if verbose {
            println!(
                "CUSTOM SEGMENTATION: Added final sentence: {}",
                sentences.last().unwrap()
            );
        }
    }
    ...
    if verbose {
        println!(
            "CUSTOM SEGMENTATION: Created {} sentences total",
            sentences.len()
        );
    }
        ...
        match &mode {
            ...
            Mode::Stream => {
                ...
                while let Some(line) = lines.next_line().await? {
                    ...
                    match tts.tts_raw_audio_with_phonemes_option(&final_text, &lan, &style, speed, initial_silence, auto_detect, force_style, phonemes) {
                        Ok(raw_audio) => {
            ...
            Mode::Pipe { output_path } => {
                ...
                loop {
                    ...
                    for (i, sentence) in complete_sentences.iter().enumerate() {
                        ...
                        match tts.tts_raw_audio_with_phonemes_option(
                            &final_preprocessed,
                            &session_language,
                            &session_style,
                            speed,
